...
**bookmarkedAt** now means when the owner bookmarked the post on X, not when the post was published — required for incremental sync. After each successful sync the server stores the newest bookmark timestamp seen as `lastBookmarkSyncAt`; the extension stops paginating once entries are at or before that point.
