...
The MVP stores only shot **metadata + X image URLs** (`pbs.twimg.com/...?name=<size>`) and hotlinks them for display; it does not download image bytes. This contradicts the durable-archive goal at the _pixel_ level — a deleted post or suspended author yields a broken image, and hosting later does not recover images that have already 404'd. We accept this for the MVP because it is zero-storage and gives sized thumbnail variants for free via the `name=` param.
