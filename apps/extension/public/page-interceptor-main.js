...
(() => {
  ...
  function parseBookmarksRequestUrl(requestUrl) {
    ...
    try {
      return {
        ...
        ...fieldTogglesRaw ? {
          fieldToggles: JSON.parse(fieldTogglesRaw)
        } : {},
        operation: match[2],
        queryId: match[1],
  ...
  function installPageInterceptor(bridgeSecret2) {
    if (!bridgeSecret2) {
      return;
    }
  ...
  function isBookmarksApiUrl(requestUrl) {
    ...
    return /\/Bookmarks(?:\?|$)/i.test(requestUrl) || /BookmarkSearchTimeline/i.test(requestUrl) || /\/Bookmark/i.test(requestUrl);
  }
  ...
  // src/page-interceptor-main.ts
  var bridgeSecret = document.documentElement?.dataset.signetsBridge;
  if (bridgeSecret) {
